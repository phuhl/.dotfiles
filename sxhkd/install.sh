...
echo ''
if [ X"$os" = X"u" ]; then
    echo 'Installing sxhkd for Ubuntu-like systems'
    echo ''
    sudo rm .tmp -r
    mkdir .tmp
...
if [ X"$os" = X"a" ]; then
    echo 'Installing sxhkd for Arch systems'
    echo ''
    sudo rm .tmp -r
    mkdir .tmp
    git clone https://github.com/baskerville/sxhkd.git .tmp
    cd .tmp
    sudo pacman -S libxcb xcb-util xcb-util-keysyms xcb-util-wm gcc make --noconfirm --needed
